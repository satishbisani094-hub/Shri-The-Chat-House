...
import { Flame, MessageSquare, PhoneCall, Sparkles, TrendingUp, HelpCircle } from 'lucide-react';
...
export default function PopularMenu() {
  ...
  const getWhatsAppOrderLink = (itemName: string) => {
    const text = encodeURIComponent(`Hello Shri The Chat House! I am looking at your online menu and would love to order: *${itemName}*. Please confirm availability!`);
    return `https://wa.me/919963233899?text=${text}`;
  };
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-12 sm:mb-16">
          {categories.map((cat) => (
            <button
              ...
              className={`px-5 py-3 rounded-xl text-xs sm:text-sm font-extrabold tracking-wide transition-all duration-300 transform border ${
          ...
          {filteredItems.map((item) => (
            ...
            >
              ...
              <div className="p-6 flex-grow flex flex-col justify-between space-y-4">
                ...
                {/* Order Shortcut Buttons */}
                <div className="pt-2 border-t border-stone-100 flex items-center gap-2">
                  <a
                    id={`menu-order-wa-btn-${item.id}`}
                    href={getWhatsAppOrderLink(item.name)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 flex items-center justify-center space-x-1 bg-green-500 hover:bg-green-600 text-white py-2 rounded-xl text-xs font-bold shadow-md hover:shadow-lg transition-all"
                  >
                  ...
                  <a
                    ...
                    className="px-3.5 bg-orange-500 hover:bg-orange-600 text-white py-2 rounded-xl text-xs font-bold shadow-md flex items-center justify-center"
